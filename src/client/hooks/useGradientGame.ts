...
import { handleGameCompletion } from "../utils/Scoring";
...
const useGradientGame = () => {
  ...
  useEffect(() => {
    ...
    setUser(
      Array.from({ length: g.length }, (_, i) =>
        CORNERS.includes(i)
          ? { col: g[i] || null, correct: true, turns: 0 }
          : { col: null, correct: false, turns: 0 }
      )
    );
  }, []);
  ...
  const takeTurn = (index: number) => {
    ...
    setUser((prev) => {
      ...
      next[index] = {
        ...placed,
        turns: (prev[index]?.turns || 0) + 1,
        correct: placed?.col === gradient[index],
        col:
          placed && prev[index]?.col === placed.col
            ? null
            : (placed?.col ?? null),
      };
      return next;
